...
import type { EmotionMarkers, LanguageCode, SpeechRole, Side } from '@shared/types';
...
class EmotionDetectorService {
  ...
  private emotionHistory: Map<string, EmotionMarkers[]> = new Map();
  ...
  private buildDetectionPrompt(
    ...
    targetLanguage: LanguageCode,
    context: EmotionContext,
    previousEmotion?: EmotionMarkers
  ): string {
...
Only return the JSON, no other text.`;
  ...
  private validateEmotion(emotion: string): EmotionMarkers['dominantEmotion'] {
    const validEmotions: EmotionMarkers['dominantEmotion'][] = [
      'confident', 'passionate', 'aggressive', 'measured', 'uncertain', 'neutral'
    ];
    if (validEmotions.includes(emotion as EmotionMarkers['dominantEmotion'])) {
      return emotion as EmotionMarkers['dominantEmotion'];
    }
    return 'neutral';
  }
   ...
   */
  private updateHistory(speakerId: string, emotion: EmotionMarkers): void {
