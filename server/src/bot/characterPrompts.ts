...
import type { BotCharacter, SpeechRole, Side, Argument } from '@shared/types';
...
const CHARACTER_PERSONAS: Record<BotCharacter, string> = {
...
const SPEECH_INSTRUCTIONS: Record<SpeechRole, string> = {
  ...
  if (speech !== 'AC' && speech !== 'NC') {
    if (opponentArguments.length > 0) {
      ...
      for (const arg of opponentArguments) {
        ...
        if (arg.warrant) prompt += `  Warrant: ${arg.warrant}\n`;
      }
      prompt += '\n';
    }
...
function getRelevantSpeeches(currentSpeech: SpeechRole): SpeechRole[] {
...
export function getSpeechDuration(speech: SpeechRole): number {
  switch (speech) {
