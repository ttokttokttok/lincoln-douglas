...
interface ParticipantSession {
  ...
  // Current WebSocket client ID (changes on reconnect)
  currentClientId: string | null;
  // Disconnect grace period timer
  disconnectTimer: NodeJS.Timeout | null;
  // Whether participant is in grace period (disconnected but not yet removed)
...
class SessionManager {
   ...
   */
  getSession(sessionToken: string): ParticipantSession | undefined {
   ...
   */
  getSessionByParticipant(participantId: string): ParticipantSession | undefined {
   ...
   */
  getSessionByClient(clientId: string): ParticipantSession | undefined {
  ...
  handleDisconnect(
    clientId: string,
    onGracePeriodExpired: (session: ParticipantSession) => void
  ): { session: ParticipantSession; startedGracePeriod: boolean } | null {
    const token = this.clientToSession.get(clientId);
   ...
   */
  private reactivateSession(sessionToken: string, newClientId: string): void {
    const session = this.sessions.get(sessionToken);
    if (!session) return;
   ...
   */
  removeRoomSessions(roomId: string): void {
   ...
   */
  isParticipantInGracePeriod(participantId: string): boolean {
    const session = this.getSessionByParticipant(participantId);
   ...
   */
  getRoomSessions(roomId: string): ParticipantSession[] {
